...
@Component({
  selector: 'app-create-external-table',
  templateUrl: './create-external-table.component.html',
  styleUrls: ['./create-external-table.component.scss']
})
export class CreateExternalTableComponent implements OnInit {
  default_select = null
  public emitData: any;
  containers: any[] = [];
  dataTypes!: any[];
  ...
  addFieldArray(): void {
    this.FiledArray = this.createETForm.get('FieldArray') as FormArray;
  ...
  get FieldArrayControls() {
    return this.createETForm.get('FieldArray') as FormArray;
  }
    ...
    this.dataAnalysisService.GetBlobSchema(this.getBlobSchemaForm.value).subscribe(
      data => {
        if (data.length === 0) {
          ...
          this.GetBlobSchema_loading = false;
          this.createETForm.get('Name')?.patchValue(this.blob_next.split('.')[0].toUpperCase());
          let i = 0;
          this.FiledArray = this.createETForm.get('FieldArray') as FormArray;
          data.forEach((item) => {
            this.addFieldArray();
  ...
  onSubmitCreateExternalTableForm() {
    this.CreateView_loading = true;
    this.createETForm.patchValue({
      FileSystem: this.fileSystem_Next,
      Path_Blob: this.blob_next
    });
