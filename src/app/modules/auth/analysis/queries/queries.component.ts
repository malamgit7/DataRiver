...
export class QueriesComponent implements OnInit {
  ...
  getScheduleDataToUpdate(QueryId: string) {
    this.analysisService.GetScheduleDataToUpdate(QueryId).subscribe(
      (res: any) => {
        const _datetime = new Date(res[0].scheduledStartDateTime);
        const _formated = this.datepipe.transform(_datetime, 'YYYY-MM-dd\THH:mm');

        this.scheduleQueryForm.patchValue({
          QueryId: res[0].queryId,
          ScheduledStartDateTime: _formated,
